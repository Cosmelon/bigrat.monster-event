...
scoreboard players set #16 sp_numsb 16
execute if score .tools sp_main matches 1 as @a[tag=player,gamemode=adventure] if score @s sp_numsb >= #16 sp_numsb run item replace entity @a[tag=player] container.35 with snow_block
clear @a[tag=player,scores={sp_numsb=..15}] snow_block

# weakness when not holding stick
