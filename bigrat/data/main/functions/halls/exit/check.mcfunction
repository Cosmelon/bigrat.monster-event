...
execute if entity @a[team=Red,tag=hallsAlive,distance=..3,sort=nearest,limit=1] unless score ~redCooldown halls_keys matches 1.. if score ~capt_red halls_keys matches 7 run schedule function main:halls/exit/redsuccess 3s replace
execute if entity @a[team=Red,tag=hallsAlive,distance=..3,sort=nearest,limit=1] unless score ~redCooldown halls_keys matches 1.. unless score ~capt_red halls_keys matches 7 run tellraw @a[team=Red] {"text":"» Failed: Not enough keys!","color":"red"}
